...
if (
	...
	console.log(
		'Cần cung cấp DATABASE_URI, DATABASE_NAME, DATABASE_PRODUCTS_COLLECTION, DATABASE_USERS_COLLECTION trong biến môi trường',
	);
} else {
	// Cấu hình kết nối tới MongoDB
	const uri = DATABASE_PASSWORD
		? DATABASE_URI.replace('<db_password>', DATABASE_PASSWORD)
		: DATABASE_URI;
	const configs = {
		connect: {
			uri: uri,
	...
	clientPromise
		.then(() => {
			...
			console.log(
				`Bộ sưu tập: ${[DATABASE_PRODUCTS_COLLECTION, DATABASE_USERS_COLLECTION].join(', ')}`,
			);
			console.log(
				'_________________________________________________________________________________________\n',
			);
	...
	productsCollection = db.collection(DATABASE_PRODUCTS_COLLECTION);
	usersCollection = db.collection(DATABASE_USERS_COLLECTION);
}
